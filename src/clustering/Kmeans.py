...
def _assign_clusters(X, centers):
    ...
    """
    dist2cents = scipy.spatial.distance.cdist(X, centers, metric='cosine')
...
def _cal_dist2center(X, center):
    ...
    """
    dmemb2cen = scipy.spatial.distance.cdist(X, center.reshape(1,X.shape[1]), metric='cosine')
    #print(dmemb2cen)
...
class KMeans(object):
    ...
    def __init__(self, n_clusters=2, n_trials=10, max_iter=5, tol=1e-4):
